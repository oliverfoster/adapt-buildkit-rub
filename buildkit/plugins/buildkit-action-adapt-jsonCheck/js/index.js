...
class Plugin {
	...
	setupEventListeners() {
		events.on("action:run:jsoncheck", (options, start, end) => { this.onActionRun(options, start, end); });
	}
	...
	onActionRun(options, start, end) {
        ...
        if (options.src.indexOf("**") || options.src.indexOf("*")) {
            ...
            var tree = treecontext.Tree(base, ".").populate();
        ...
        if (hasErrored) {
            return end('Oops, looks like you have some json errors.')
        }
        ...
        function check(options) {
            ...
            if (orphanedIds.length > 0) {
                hasErrored = true;
                logger.warn('Orphaned _ids: ' + orphanedIds);
            }

            if (missingIds.length > 0) {
                hasErrored = true;
                logger.warn('Missing _ids: ' + missingIds);
            }

            if (emptyIds.length > 0) {
                hasErrored = true;
                logger.warn('Empty _ids: ' + emptyIds);
            }

            if (duplicateIds.length > 0) {
                hasErrored = true;
                logger.warn('Duplicate _ids: ' + duplicateIds);
            }
