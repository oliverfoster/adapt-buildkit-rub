...
var pub = module.exports = {
    ...
    _expandMimeTypes: function () {
		for (var mt in pub._mimeTypes) {
		    for (var i = 0, l = pub._mimeTypes[mt].length; i < l; i++) {
		        pub._mimeTypesByExtension[ pub._mimeTypes[mt][i] ] =  mt;
		    }
		}
	},
		...
		function request(req, res) {
		    ...
		    if (path.dirname(stat.path) === "/__server__/poll") {
		    	if (stat.search) {
		    		...
		    		if (!isNaN(timestamp)) {
		    			for (var i = 0; i < pub._reloadItems.length; i++) {
		    				if (pub._reloadItems[i].lastEvent > timestamp) {
		    					res.writeHead(200, {"Content-Type": "text/json"});
						        res.write(JSON.stringify(pub._reloadItems[i]));
						        res.end();
						        return;
		    				}
		    			}
		    	...
		    	var mimeType = pub._mimeTypesByExtension[path.extname(filename).split(".")[1]];
